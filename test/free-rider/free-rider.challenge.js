...
describe("[Challenge] Free Rider", function () {
	...
	it("Exploit", async function () {
		...
		// Flashloan 15 WETH
		// Swap WETH for ETH
		// Call the marketplace to buy all the NFTs, msg.value being 15 ETH
		// Swap back to WETH
		// Repay flashloan
		// Deploy WETH contract
		...
		await expect(
			attack.connect(attacker).attack({ value: ethers.utils.parseEther("0.4") })
		).to.not.be.reverted;

		await expect(attack.connect(attacker).transferToBuyer()).to.not.be.reverted;
	});
