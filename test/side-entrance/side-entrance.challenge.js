...
describe("[Challenge] Side entrance", function () {
	...
	it("Exploit", async function () {
		...
		const Attack = await ethers.getContractFactory("attackSide", attacker);
		this.attack = await Attack.deploy(this.pool.address);

		await expect(this.attack.connect(attacker).attack()).to.not.be.reverted;

		await expect(this.attack.connect(attacker).withdraw()).to.not.be.reverted;
	});
